...
public class SocketClient {
    ...
    void OnConnect(IAsyncResult asr) {
        try
        {
           /* TcpClient t = (TcpClient)asr.AsyncState;
            t.EndConnect(asr);
            */
            ...
            NetworkManager.AddEvent(Protocal.Connect.ToString(), new LuaByteBuffer(new byte[] { }));

        } catch(Exception e) {
            byte[] bytes = System.Text.Encoding.Default.GetBytes(e.Message);
            NetworkManager.AddEvent(Protocal.Exception.ToString(), new LuaByteBuffer(bytes));
        }
    }
